...
Open an interactive picker over snapshot or close events. The picker is a Bubble Tea TUI that shows each snapshot's full session → window → pane tree before you restore it, and exposes the smart-restore filter as live footer toggles in `--kind=snapshot`.

![Both pickers: browse snapshots, then close a pane and see what it had on screen before bringing it back](demo/pick.gif)

- `--kind=snapshot` (default) — snapshots on the left, the selected snapshot's tree in the middle, the selected pane's captured scrollback on the right. The preview needs 120 columns; narrower than that it drops to two panes, and since the binds open the picker in a popup at 90% width, a terminal under ~134 columns never shows it. Toggle `s` to skip idle shells, `d` to skip sessions already running (shown collapsed with a `(running)` tag), `a` to dim snapshots older than 24h. `↑`/`↓` walk the tree; `Alt+h`/`Alt+j`/`Alt+k`/`Alt+l` (plus `PgUp`/`PgDn`) pan the preview scrollback without moving the cursor.
- `--kind=close` — a flat list of closes, newest first, under two headings: this session, then everything else. Each row is one close (or a collapsed run of identical ones, tagged `×N`) rendered as columns: a marker, what was closed, the directory it was in, its name, where `↵` would put it back, and its age, so a `Ctrl+D` cascade takes one line rather than ten. There is nothing to expand — `↑`/`↓` and `↵` are the whole story, with `Alt+h`/`Alt+j`/`Alt+k`/`Alt+l` panning the preview's scrollback; the filter toggles stay out of the footer here: a close is restored whole, so `s`/`d`/`a` and the kept/skipped counter belong to the snapshot picker alone. The right-hand column previews what the row under the cursor would reopen: the captured scrollback of every pane the close took down. It follows the cursor with no Tab. The two columns sit side by side from ~163 columns up (the list's own width tops out at 100 and every further cell goes to the preview); below that the preview moves underneath the full-width list rather than being squeezed, and under 80 columns it drops away entirely. The closed panes are read out of the snapshot the close was diffed against, which is where their scrollback comes from. Used by `prefix + U`.
- A snapshot saved within `min_save_interval` of the previous one is stored without scrollback on purpose (it keeps a restore storm's hook-per-window saves cheap), and the snapshot picker's preview reads `(scrollback skipped — saved within min_save_interval)` for its panes. A close resolved against one of those does not say the same, because resolving it already looked back past the downgraded snapshot for an earlier capture of the same pane; when that finds nothing too, the per-pane block reads `(nothing captured for this pane before it closed)` — usually a pane that was opened and closed between two saves.
- The close list marks each row's scope with a Nerd Font codicon (`nf-cod-terminal`, `nf-cod-window`, `nf-cod-multiple_windows`, `nf-cod-layers`). On a terminal whose font has none of them, `set -g @remux_ascii_glyphs on` swaps in the geometric stand-ins `▪ ◫ ▣ ◇` instead.
- The close list can render extra columns sourced from tmux window options another tool set — an issue id, a PR number, whatever that tool stamps. Declare them with `@remux_columns`, a comma-separated list of `option:role:max` triples; roles are `id` (compact identity, left-anchored), `badge` (short marker), and `text` (the flexible title column, which ignores `max`). For example, `set -g @remux_columns '@issue_id:id:10,@pr_number:badge:6,@issue_title:text:0'`. Declaring a column adds its option to the snapshot allow-list automatically, so no separate capture config is needed. A malformed entry is skipped rather than blanking the picker. Unset by default — tmux-remux doesn't assume any particular option schema.
- Windows a plugin opens and closes on its own clutter the close list with closes you never made. Name them with `@remux_ignore_windows`, a comma-separated list of window names — for example, `set -g @remux_ignore_windows '[fingers],scratch-*'`. An entry is a glob only when it contains `*` or `?`; otherwise it matches literally, so a name with brackets works as written. Their closes are still recorded, just kept out of the list and counted in its hidden-count line. Unset by default.
- Window and pane decoration — the options snapshotted and replayed on restore, cosmetic ones included (`pane-border-style` and friends) — are configurable the same way: `@remux_decoration_options` (window-scoped, comma-separated option names, default `@crew_name,@crew_color,pane-border-style,pane-active-border-style,pane-border-format,pane-border-status`) and `@remux_pane_decoration_options` (pane-scoped, default `@crew_role,@crew_role_color,@crew_state,pane-border-style,pane-active-border-style,pane-border-format`). Every allow-listed option, on both lists, is captured only when it's set locally on that window or pane — never when it's only inherited from a session or global default — so restore never pins an inherited theme value onto a window or pane that never had it set directly. This is a behavior change from earlier versions: a `@remux_columns`-declared option set only at session or global scope (not on the window itself) used to render in the close list via format-string expansion and will now stop rendering there, since capture no longer follows inheritance. The picker's own column renderer (`internal/picker/view.go`, `w.Decoration[col.Option]`) is unaffected by the widened default allow-list — it only ever reads the keys `@remux_columns` declares.

Tab switches focus between panes in the snapshot picker. `?` shows the full keymap. `enter` restores; `esc` cancels. The mouse works too: the wheel scrolls whatever pane is under the pointer — the list's cursor, the tree, or the preview's scrollback — and a left-click selects a list row or tree node.
