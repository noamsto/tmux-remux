...
- Pre-commit runs gofmt, golangci-lint, govet, typos. The typos hook rejects the
  `-eable` spelling of **unparsable**; use that spelling in comments and docs.
...
- Produces: `func Render(g Grid, w, h int, label func(int) string, marked func(int) bool) string`
  — the size guard plus the constants `minMapWidth = 24`, `minMapHeight = 6` —
  delegating to an unexported `render` with the same signature that always
  draws. The split exists so the art can be asserted at small sizes without
  tripping the guard. Labels and marking are wired in Task 3; pass `nil` until
  then.
...
All of this goes in `render`, not `Render` — the tests call `render` directly
with nil callbacks, so the defaults have to be inside it. Add them as its first
statements, before `boxes := scale(...)`:
